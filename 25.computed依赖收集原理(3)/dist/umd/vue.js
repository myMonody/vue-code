(function (global, factory) {
  ...
  function popTarget() {
    stack.pop(); // Dep.target = null; // 将变量删除。
  ...
  function initComputed(vm) {
    let computed = vm.$options.computed; // 1，需要一个watcher 2,还需要通过defineProperty 3,dirty

    const watcher = vm._computedWatcher = {};

    for (let key in computed) {
      const userDef = computed[key];
      const getter = typeof userDef == "function" ? userDef : userDef.get; // 给每一个属性都增加一个watcher。
